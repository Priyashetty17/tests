...
func (prq *ProjectsResourceQuotaTestSuite) TestProjectWithResourceQuota() {
	...
	err = kwait.PollUntilContextTimeout(context.TODO(), defaults.FiveHundredMillisecondTimeout, defaults.TenSecondTimeout, true, func(ctx context.Context) (bool, error) {
		checkErr := deploymentapi.VerifyDeploymentStatus(standardUserClient, prq.cluster.ID, firstNamespace.Name, createdSecondDeployment.Name, "ReplicaFailure", "FailedCreate", "forbidden: exceeded quota", 0)
		if checkErr != nil {
	...
	require.NoError(prq.T(), err)
	err = kwait.PollUntilContextTimeout(context.TODO(), defaults.FiveHundredMillisecondTimeout, defaults.TenSecondTimeout, true, func(ctx context.Context) (bool, error) {
		checkErr := deploymentapi.VerifyDeploymentStatus(standardUserClient, prq.cluster.ID, secondNamespace.Name, createdDeployment.Name, "ReplicaFailure", "FailedCreate", "forbidden: exceeded quota", 0)
		if checkErr != nil {
			return false, checkErr
		}
	...
	err = deploymentapi.WaitForDeploymentActive(standardUserClient, prq.cluster.ID, firstNamespace.Name, updatedSecondDeployment.Name)
	require.NoError(prq.T(), err)
}
...
func (prq *ProjectsResourceQuotaTestSuite) TestQuotaDeletionPropagationToExistingNamespaces() {
	...
	log.Info("Verify that the namespace does not have the annotation: field.cattle.io/resourceQuota.")
	err = kwait.PollUntilContextTimeout(context.TODO(), defaults.FiveHundredMillisecondTimeout, defaults.TenSecondTimeout, true, func(ctx context.Context) (bool, error) {
		checkErr := namespaceapi.VerifyAnnotationExistsInNamespace(standardUserClient, prq.cluster.ID, createdNamespace.Name, projectapi.ResourceQuotaAnnotation, false)
		if checkErr != nil {
			return false, checkErr
		}
...
func (prq *ProjectsResourceQuotaTestSuite) TestMoveNamespaceFromNoQuotaToQuotaProject() {
	...
	err = kwait.PollUntilContextTimeout(context.TODO(), defaults.FiveHundredMillisecondTimeout, defaults.TenSecondTimeout, true, func(ctx context.Context) (bool, error) {
		checkErr := deploymentapi.VerifyDeploymentStatus(standardUserClient, prq.cluster.ID, movedNamespace.Name, updatedDeployment.Name, "ReplicaFailure", "FailedCreate", "forbidden: exceeded quota", 2)
		if checkErr != nil {
			return false, checkErr
		}
...
func (prq *ProjectsResourceQuotaTestSuite) TestMoveNamespaceFromQuotaToNoQuotaProject() {
	...
	log.Info("Verify that the namespace does not have the annotation: field.cattle.io/resourceQuota.")
	err = kwait.PollUntilContextTimeout(context.TODO(), defaults.FiveHundredMillisecondTimeout, defaults.TenSecondTimeout, true, func(ctx context.Context) (bool, error) {
		checkErr := namespaceapi.VerifyAnnotationExistsInNamespace(standardUserClient, prq.cluster.ID, movedNamespace.Name, projectapi.ResourceQuotaAnnotation, false)
		if checkErr != nil {
			return false, checkErr
		}
...
func (prq *ProjectsResourceQuotaTestSuite) TestMoveNamespaceWithDeploymentTransition() {
	...
	err = kwait.PollUntilContextTimeout(context.TODO(), defaults.FiveHundredMillisecondTimeout, defaults.TenSecondTimeout, true, func(ctx context.Context) (bool, error) {
		checkErr := deploymentapi.VerifyDeploymentStatus(standardUserClient, prq.cluster.ID, createdNamespace.Name, createdDeployment.Name, "ReplicaFailure", "FailedCreate", "forbidden: exceeded quota", 0)
		if checkErr != nil {
			return false, checkErr
		}
